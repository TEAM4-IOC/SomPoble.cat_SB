...
/**
 * Servicio que procesa y agrega las métricas para el panel de control de empresas.
 * Se encarga de consultar los datos de reservas, ingresos y clientes, tanto totales como desglosados,
...
public class MetricasServiceImpl implements MetricasService {
     ...
     * @throws RuntimeException si la empresa no es encontrada por su identificador fiscal.
     */
    public PanelMetricasDTO obtenerMetricas(String identificadorFiscal, LocalDate fechaInicio, LocalDate fechaFin) {
        ...
        if (fechaInicio == null) fechaInicio = LocalDate.now().minusMonths(6);
        ...
        List<PanelMetricasDTO.ServicioResumenDTO> servicios = reservaRepository.obtenerMetricasPorServicio(
                empresa.getIdEmpresa(), fechaInicio, fechaFin);

 ...
 * <p>
 * Utiliza Spring Mail y Thymeleaf para enviar emails basados en una plantilla HTML.
 * </p>
 */
@Service
public class EmailServiceImpl implements EmailService {
     ...
     *
     * @param notificacion La notificación que contiene los detalles del mensaje.
     */
    @Override
    ...
    public String generateSubject(Notificacion.TipoNotificacion tipo) {
        return switch (tipo) {
            case INFORMACION -> "Información sobre su reserva - SomPoble";
            case ADVERTENCIA -> "⚠️ Acción requerida - SomPoble";
            case ERROR -> "Error en actualizar reserva - SomPoble";
        };
    }

    /**
     * Obtiene el email del destinatario (cliente o empresario) asociado a la notificación.
     *
     * @param notificacion La notificación que contiene los datos del destinatario.
    ...
    /**
     * Envía un correo electrónico utilizando una plantilla HTML y un recurso embebido.
     * <p>
     * Este método recibe un objeto {@link EmailDTO} que contiene el destinatario, asunto y mensaje del correo.
     * Se utiliza el motor de plantillas {@code TemplateEngine} para procesar el contenido dinámico del correo
     * y se añade una imagen (logo) de forma embebida en el cuerpo del correo.
     * </p>
     * <p>
     * Si ocurre algún error durante la creación o el envío del correo, se lanza una {@link RuntimeException}.
     * </p>
     *
     * @param email el objeto {@link EmailDTO} que contiene los datos necesarios para enviar el correo
     * @throws MessagingException si ocurre un error relacionado con el manejo del mensaje
     */
	...
	/**
	 * Envía recordatorios automáticos por correo electrónico para las reservas próximas.
	 * <p>
	 * Este método consulta la base de datos en busca de reservas cuya fecha esté entre el día actual 
	 * y el siguiente día. Por cada reserva encontrada, se genera una notificación de tipo "información" 
	 * y se envía un correo electrónico al cliente asociado a la reserva.
	 * </p>
	 * <p>
	 * El envío de correos se realiza mediante el método {@code sendNotificationEmail(Notificacion notificacion)}.
	 * </p>
	 */
	@Override
	 ...
	 * <p>
	 * El correo se envía a una dirección específica con un asunto y un cuerpo de texto simples,
	 * sin utilizar plantillas ni contenido HTML. Se utiliza para verificar que el servicio de
	 * envío de correos esté funcionando correctamente.
	 * </p>
	 * <p>
	 * En caso de error durante el proceso de envío, se captura la excepción y se imprime
	 * la traza del error en la consola.
	 * </p>
	 */
